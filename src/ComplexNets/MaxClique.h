...
public:
    ...
    MaxCliqueVisitor(MaxClique<Graph, Vertex>& observer)
        : maxCliqueObserver(observer) {}
...
public:
    ...
    MaxCliqueMap(MaxClique<Graph, Vertex>& observer)
        : maxCliqueObserver(observer) {}
...
public:
	...
	int getMaxCliqueSize(Vertex* vertex){
		...
		for(a_iterator iterator2 = as[vertex]->begin(); 
							iterator2 != as[vertex]->end(); iterator2++){
			if((*iterator2).second && (*iterator2).second->size()>max_size){
				max_size =  (*iterator2).second->size();
				delete ids;
				ids = new std::list<int>();
				
				for(L_iterator it = (*iterator2).second->begin();it != (*iterator2).second->end();it++ ){
	...
	virtual std::list<int> getMaxCliqueList(){
		...
		for(MaxCliqueIterator iterator1 = MaxCliqueIterator(container); !iterator1.end(); iterator1++){
...
public:
    ...
    MaxCliqueExactVisitor(MaxCliqueExact<Graph, Vertex>& observer)
        : maxCliquenObserver(observer) {}
...
public:
    ...
    MaxCliqueExactMap(MaxCliqueExact<Graph, Vertex>& observer)
        : maxCliqueObserver(observer) {}
...
public:
    ...
    MaxCliqueExact(Graph& graph,int max_time):maxTime(max_time),timeouted(false), start(time(NULL))
    {
        calculateMaxClique(graph);
    }
            ...
            if (x->getVisited())
			{
				...
				if(as[x] == NULL){
					...
					if(as[v] ==NULL){
						as[v] = new a_type();
						_new = true;
					}else for(a_iterator it3 = as[v]->begin();it3!=as[v]->end();it3++){
						L_type* B = (*it3);
						if(B->size()<L->size()){
							if(include(B,L)){
								it3 = as[v]->erase(it3);
				...
				}else for(a_iterator it2 = as[x]->begin();it2!=as[x]->end();it2++){
					L_type* A= (*it2);
					bool _new = false;
					bool _old = false;
...
private:
    ...
    void calculateMaxClique(Graph& graph)
    {
	    ...
	    TraverserOrdered<Graph, Vertex, 
				MaxCliqueExactVisitor<Graph, Vertex>,DegreeCompare >::
