/*
  ...
  gengetopt -i ./ComplexNetsCmd/cmdline.ggo 
...
cmdline_parser_required2 (struct gengetopt_args_info *args_info, const char *prog_name, const char *additional_error)
{
  ...
  if (args_info->p_given && ! args_info->erdos_given)
    {
      fprintf (stderr, "%s: '--p' ('-p') option depends on option 'erdos'%s\n", prog_name, (additional_error ? additional_error : ""));
      error = 1;
    }
  if (args_info->m0_given && ! args_info->erdos_given)
    {
      fprintf (stderr, "%s: '--m0' ('-c') option depends on option 'erdos'%s\n", prog_name, (additional_error ? additional_error : ""));
      error = 1;
    }
